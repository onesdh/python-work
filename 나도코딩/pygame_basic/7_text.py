...
# 적 캐릭터
enemy = pygame.image.load("C:\\Users\\onesd\\Desktop\\python work\\나도코딩\\pygame_basic\\enemy.png")
enemy_size = character.get_rect().size  #이미지의 크기를 구해줌
enemy_width = enemy_size[0] #캐릭터의 가로크기
enemy_height = enemy_size[1] #캐릭터의 세로크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) #화면가로의 절반크기에 해당하는 곳에 위치
enemy_y_pos = (screen_height/2) - (enemy_height / 2) # 화면세로크기 가장 아래에 해당하는 곳에 위치
...
while running :
    ...
    #타이머 집어 넣기
    #경과 시간 계산
    elapsed_timme = (pygame.time.get_ticks() - start_ticks) / 1000
    # 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 표시

    timer = game_font.render(str(int(total_time - elapsed_timme)), True, (255, 255, 255))
    # 출력할 글자, True, 글자색상
    screen.blit(timer, (10,10))

    # 만약 시간이 0 이하이면 게임 종료
    if total_time - elapsed_timme <= 0:
        print("타임아웃")
        running = False
